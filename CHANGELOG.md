...
- ***(SECURITY)*** Refuse a request whose method is not an [RFC 9110 §5.6.2](https://www.rfc-editor.org/rfc/rfc9110#section-5.6.2) token with `400 Bad Request` [#820](https://github.com/kemalcr/kemal/issues/820). Kemal keys its routing tree on the method concatenated with the path and Crystal's parser passes the method through unvalidated, so a method carrying a `/` chose where the one ended and the other began: `GET/admin` + `/secret` produced the same key as `GET` + `/admin/secret` and reached that route, while `use "/prefix"`, `only`/`exclude` and `before_*`/`after_*` all match on `request.path` and saw only `/secret` - an unauthenticated bypass of path-scoped authorization on any route, in a single request. `GET/account` + `/` left `request.path` as `/`, defeating exact-path guards too. The new `Kemal::MethodValidationHandler` refuses such a method behind the log and exception handlers, so the refusal is logged and rendered by a registered `error 400`, and ahead of the static file, custom and filter handlers, so none of them ever sees it; `Kemal::RouteHandler` asks again where the key is built, covering a chain assembled by hand through `Kemal.config.handlers=`. The refusal closes the connection (`Connection: close`), since a request line Kemal and an intermediary read differently is what request smuggling is built on. Unfamiliar but well-formed methods are untouched: a `PROPFIND` still reaches the router and gets its answer. The handler takes one chain position, so an explicit `use handler, position` past the error handler now lands one place later. Thanks @yvzkr for the report :pray:
